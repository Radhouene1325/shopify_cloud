...
export async function compressImage(
    ...
    if (!response.ok && format === "avif" && !cfImageDomain) {
        console.warn("[Image Optimizer] AVIF failed, trying WebP fallback");
        const fallbackUrl = optimizedUrl.replace("format=avif", "format=webp");
        const fallbackRes = await fetch(fallbackUrl);
        if (fallbackRes.ok) {
            const ab = await fallbackRes.arrayBuffer();
            return {
                compressedBuffer: new Uint8Array(ab),
                contentType: "image/webp",
                format: "WebP",
                originalSize: originalSize || ab.byteLength,
                compressedSize: ab.byteLength,
            };
        }
    }
    ...
    if (format === "avif" && originalSize > 0 && compressedSize >= originalSize * 0.95 && !cfImageDomain) {
        console.warn("[Image Optimizer] AVIF larger than original, trying WebP");
        const fallbackUrl = optimizedUrl.replace("format=avif", "format=webp");
        const fallbackRes = await fetch(fallbackUrl);
        if (fallbackRes.ok) {
            const ab = await fallbackRes.arrayBuffer();
            const webpSize = ab.byteLength;
            if (webpSize < compressedSize) {
                return {
                    compressedBuffer: new Uint8Array(ab),
                    contentType: "image/webp",
                    format: "WebP",
...
export const action = async ({ request, context }: ActionFunctionArgs) => {
    ...
    try {
        ...
        const resourceUrl = await uploadToShopifyCDN(admin, compressedBuffer, contentType);
