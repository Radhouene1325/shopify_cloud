...
//       '        </div>',
//       '        <div itemprop="offers" itemscope itemtype="https://schema.org/Offer">',
//       '          <meta itemprop="url" content="https://platinumshop.it/products/{{handle}}">',
//       '          <meta itemprop="priceCurrency" content="{{currencyCode}}">',
//       '          <meta itemprop="price" content="{{min_amount}}">            <!-- usa min_amount come prezzo -->',
//       '          <link itemprop="availability" href="https://schema.org/InStock">',
//       '        </div>',
//       '    - I placeholder ({{...}}) vanno sostituiti con i VALORI REALI dai campi del prodotto:',
...
//       '        * {{totalInventory}} → usa il campo "totalInventory" (per disponibilità)',
//       '        * {{tracksInventory}} → usa il campo "tracksInventory"',
//       '    - Se totalInventory > 0, usa availability "InStock", altrimenti "OutOfStock".',
//       '    - Se tracksInventory è 0, puoi omettere il controllo scorte.',
//       '',
//       '  ✅ REQUISITI GOOGLE RICH RESULTS:',
...
// - Preserves all existing images with responsive wrappers
// - Follows e-commerce best practices
// - **Includes complete Schema.org microdata with ALL required fields populated from the provided product variables**
// - **Distingue nettamente short e detailed description**: la short è solo bullet points + CTA; la detailed include tutte le sezioni (sommario, caratteristiche, benefici, specifiche, tabella taglie se disponibile, immagini, CTA) **e i microdata all'inizio con i valori reali dei campi**.

// AVAILABLE PRODUCT VARIABLES (use these exact values in meta tags and content):
// - id: "${chunk.map(p => p.id).join('", "')}"
...
// - max_amount: "${chunk.map(p => p.max_amount).join('", "')}"
// - currencyCode: "${chunk.map(p => p.currencyCode).join('", "')}"
...
//     isShort
//       ? '"shortDescription": "RESPONSIVE_HTML (solo bullet points e CTA)"'
//       : '"detailedDescription": "RESPONSIVE_HTML (struttura completa con microdata all\'inizio usando i valori reali dei campi sopra)"'
//   }
// }
...
// ESEMPIO detailedDescription (CON MICRODATA E TABELLA TAGLIE - USA I VALORI REALI DEI CAMPI):
// <div itemscope itemtype="https://schema.org/Product" style="max-width:100%;overflow-wrap:break-word;word-wrap:break-word;">
...
// </div>
// <div itemprop="offers" itemscope itemtype="https://schema.org/Offer">
//   <meta itemprop="url" content="https://platinumshop.it/products/${chunk[0]?.handle || ''}">
//   <meta itemprop="priceCurrency" content="${chunk[0]?.currencyCode || 'EUR'}">
//   <meta itemprop="price" content="${chunk[0]?.min_amount || '0.00'}">
//   <link itemprop="availability" href="https://schema.org/${(chunk[0]?.totalInventory || 0) > 0 ? 'InStock' : 'OutOfStock'}">
// </div>

// <!-- CONTENUTO VISIBILE -->
// <h2 style="margin:1em 0 0.5em;line-height:1.3;">Product Overview</h2>
// <p style="margin:0.8em 0;line-height:1.6;">Experience the perfect fusion of ${chunk[0]?.vendor || 'brand'}'s legendary comfort...</p>

// <h3 style="margin:1.2em 0 0.5em;line-height:1.3;">Key Features</h3>
// <ul style="padding-left:1.2em;margin:0.8em 0;line-height:1.6;">
// <li style="margin-bottom:0.5em;">Feature 1 based on ${chunk[0]?.descreption?.substring(0, 50)}...</li>
// </ul>
...
// <tr style="border-bottom:1px solid #ddd;">
// <td style="padding:0.6em 0.8em;border-right:1px solid #ddd;font-weight:bold;width:40%;vertical-align:top;">Brand:</td>
// <td style="padding:0.6em 0.8em;width:60%;vertical-align:top;">${chunk[0]?.vendor || 'Brand'}</td>
// </tr>
// <tr style="border-bottom:1px solid #ddd;">
// <td style="padding:0.6em 0.8em;border-right:1px solid #ddd;font-weight:bold;width:40%;vertical-align:top;">Price:</td>
// <td style="padding:0.6em 0.8em;width:60%;vertical-align:top;">${chunk[0]?.currencyCode || 'EUR'} ${chunk[0]?.min_amount || '0.00'}</td>
// </tr>
// </table>
// </div>
...
// PROCESSING INSTRUCTIONS:
// 1. Analizza ogni prodotto separatamente usando i VALORI REALI forniti sopra.
// 2. Per ogni prodotto, usa questi valori esatti nei meta tag:
...
// 3. **Per shortDescription**: genera SOLO bullet points e CTA, nient'altro.
// 4. **Per detailedDescription**: 
//    - Costruisci il wrapper principale con \`itemscope\` e i meta tag usando i VALORI REALI dei campi.
//    - Includi TUTTE le sezioni: overview, key features, benefits, specifications, size chart (se disponibile), immagini, CTA.
//    - Estrai una SEO description dai primi 150-160 caratteri di descreption o generane una riassuntiva.
// 5. Avvolgi tutto in un contenitore <div> responsivo con \`itemscope\`.
// 6. Usa esclusivamente stili inline per spaziatura e layout.
// 7. Rendi le tabelle scrollabili orizzontalmente su mobile con wrapper \`overflow-x:auto\`.
// 8. Avvolgi ogni immagine in un <div> responsivo.
// 9. Preserva tutte le immagini originali dalla descreption.
// 10. Aggiungi segnali di fiducia e una CTA convincente.
// 11. Assicura la perfetta visualizzazione su dispositivi da 320px a 1920px.
...
function buildPrompt(
  ...
  if (isShort) {
    ...
    constraints = [
      ...
      '  4. <h3>Specifications</h3> con tabella responsiva a due colonne (Feature | Value).',
      '  5. Se sono presenti informazioni sulle taglie (misure, conversioni EU/US, lunghezza piede), creare una sezione <h3>Size Chart</h3> con tabella a 4 colonne (EU | US | UK | Foot Length cm) o colonne appropriate.',
      '  6. **GESTIONE IMMAGINI**: Se nel campo `descreption` sono presenti tag <img>, preservarli ESATTAMENTE come sono (stessi src, alt, attributi). NON aggiungere nuove immagini, NON modificare gli URL esistenti. Avvolgere ogni immagine in <div style="max-width:100%;margin:1em 0;"><img style="max-width:100%;height:auto;display:block;" ...></div> mantenendo TUTTI gli attributi originali.',
      '  7. Un paragrafo finale <p> con CTA (es. "Free shipping on orders over €50!").',
      '',
      '  ✅ MICRODATA SCHEMA.ORG (OBBLIGATORIO – deve superare il Google Rich Results Test):',
      '    - Tutto il contenuto visibile deve essere racchiuso in un <div> con:',
      ...
      '        <meta itemprop="name" content="{{title}}">',
      '        <meta itemprop="description" content="{{SEO_DESCRIPTION}}">   <!-- breve descrizione SEO, 150-160 caratteri -->',
      '        <!-- NOTA: Il campo url è stato rimosso, quindi NON includere itemprop="image" -->',
      '        <div itemprop="brand" itemscope itemtype="https://schema.org/Brand">',
      '          <meta itemprop="name" content="{{vendor}}">',
      '        </div>',
      '        <div itemprop="offers" itemscope itemtype="https://schema.org/Offer">',
      ...
      '    - Se totalInventory > 0, usa availability "InStock", altrimenti "OutOfStock".',
      '    - Se tracksInventory è 0, puoi omettere il controllo scorte.',
      '    - **IMPORTANTE**: NON includere <link itemprop="image"> poiché il campo url non è disponibile.',
      '',
      '  ✅ REQUISITI GOOGLE RICH RESULTS:',
      '    - Il campo `name` (title) è obbligatorio.',
      '    - Il campo `offers` (prezzo, valuta, disponibilità) è obbligatorio.',
      '    - Il campo `brand` (vendor) è obbligatorio.',
      '    - Il campo `image` è fortemente raccomandato ma NON obbligatorio se non disponibile.',
      '    - Assicurati che il prezzo sia un numero con punto (es. "89.95") e la valuta sia "EUR".',
      '',
      '  La tabella delle specifiche e quella delle taglie devono essere avvolte in un contenitore con overflow-x:auto per lo scorrimento orizzontale su mobile.',
      '  Usare SOLO stili inline per layout/responsività (margin, padding, line-height, max-width, overflow, border).',
...
- Lets Shopify theme control typography and colors
- Drives conversions through benefit-focused copy
- **Preserves all existing images from the original description exactly as they appear**
- Follows e-commerce best practices
- **Includes complete Schema.org microdata with available fields (NOTE: image field excluded as url not provided)**
- **Distingue nettamente short e detailed description**: la short è solo bullet points + CTA; la detailed include tutte le sezioni (sommario, caratteristiche, benefici, specifiche, tabella taglie se disponibile, immagini originali, CTA) **e i microdata all'inizio con i valori reali dei campi**.
...
ESEMPIO detailedDescription (CON MICRODATA - SENZA URL IMMAGINE):
<div itemscope itemtype="https://schema.org/Product" style="max-width:100%;overflow-wrap:break-word;word-wrap:break-word;">

<!-- MICRODATA INVISIBILI (MANDATORI per Google Rich Results) - USA I VALORI REALI -->
<meta itemprop="name" content="${chunk[0]?.title || 'Product Name'}">
<meta itemprop="description" content="SEO description extracted from descreption field, 150-160 chars">
<!-- NOTA: Nessun itemprop="image" perché il campo url non è disponibile -->
<div itemprop="brand" itemscope itemtype="https://schema.org/Brand">
  <meta itemprop="name" content="${chunk[0]?.vendor || 'PlatiNum'}">
</div>
<div itemprop="offers" itemscope itemtype="https://schema.org/Offer">
...
<!-- IMMAGINI ORIGINALI (preservate esattamente come sono in descreption) -->
<div style="max-width:100%;margin:1em 0;">
  <img src="URL_ORIGINALE_DALLA_DESCREPTION" style="max-width:100%;height:auto;display:block;" alt="ALT_ORIGINALE">
</div>
...
2. Per ogni prodotto, usa questi valori esatti nei meta tag:
   ...
   - currencyCode → <meta itemprop="priceCurrency" content="VALORE">
   - totalInventory → se > 0 usa InStock, altrimenti OutOfStock
   - **NON includere itemprop="image"** poiché il campo url non è disponibile
3. **Per shortDescription**: genera SOLO bullet points e CTA, nient'altro.
4. **Per detailedDescription**: 
   - Costruisci il wrapper principale con \`itemscope\` e i meta tag usando i VALORI REALI dei campi.
   - Includi TUTTE le sezioni: overview, key features, benefits, specifications, size chart (se disponibile), **immagini originali dalla descreption**, CTA.
   - Estrai una SEO description dai primi 150-160 caratteri di descreption o generane una riassuntiva.
   - **Gestione immagini**: Estrai tutti i tag <img> dal campo descreption, preserva gli attributi originali, avvolgili nel wrapper responsivo.
5. Avvolgi tutto in un contenitore <div> responsivo con \`itemscope\`.
6. Usa esclusivamente stili inline per spaziatura e layout.
7. Rendi le tabelle scrollabili orizzontalmente su mobile con wrapper \`overflow-x:auto\`.
...
- Perfect responsive on ALL devices
- **Per detailedDescription, usa SEMPRE i valori reali dei campi handle, title, vendor, min_amount, currencyCode, totalInventory nei meta tag Schema.org**
- **NON aggiungere itemprop="image" nei microdata**
- **Preserva ESATTAMENTE le immagini esistenti in descreption, solo aggiungendo wrapper responsivo**`;
}
