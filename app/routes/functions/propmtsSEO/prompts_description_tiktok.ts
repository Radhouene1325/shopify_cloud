...
type PromptProduct = {
  ...
  tracksInventory?: number | boolean;
  max_amount?: string;
  currencyCode?: string;
  min_amount?: string;
};
...
function collectImageCandidates(product: PromptProduct) {
  ...
  const description = product.descreption || product.description || product.descriptionHtml || "";
  ...
  extractImagesFromHtml(description).forEach((image) => candidates.push(image));
...
export function buildTikTokPrompt(
  input: PromptInput,
  _outputField: OutputField = "shortDescription"
): string {
...
${JSON.stringify(
    products.map((product) => ({
      ...
      description: product.descreption || product.description || product.descriptionHtml,
...
export { buildTikTokPrompt as buildPrompt };
