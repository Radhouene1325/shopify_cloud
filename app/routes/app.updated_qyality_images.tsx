...
/**
 * Safe Shopify Admin GraphQL client
 * - Exponential backoff per Throttling
...
export async function safeGraphql<T = any>(
  ...
  try {
    ...
    // Shopify Throttling: exponential backoff
    if (json.errors?.some((e) => e.message.includes("Throttled"))) {
      if (retries > 0) {
        const delay = 500 * Math.pow(2, 5 - retries); // 500ms → 1s → 2s → 4s → 8s
 ...
 * REQUIRES: Image Resizing attivo sul dominio Cloudflare
 */
export async function compressToWebP(
  ...
  // Diagnostica critica: se non è webp, Image Resizing non è attivo
  ...
  return {
    compressedBuffer: new Uint8Array(arrayBuffer),
    contentType,
  };
}
...
export const action = async ({ request, context }: ActionFunctionArgs) => {
  ...
  try {
    ...
    if (intent === "variant_image") {
      ...
      // Associate variant → new media (API 2025-01 usa mediaId)
      ...
      return json<ActionData>({
        success: true,
        type: "variant_image",
        variant: updateRes.data?.productVariantsBulkUpdate?.productVariants?.[0],
      });
    }
...
export default function ImageOptimizer() {
  ...
  // Auto-refresh dopo ottimizzazione
  useEffect(() => {
        ...
        >
          {actionData.success ? (
            <span>
              ...
              {actionData.type === "product_image" && <span> · Product image</span>}
              {actionData.type === "variant_image" && <span> · Variant image</span>}
            </span>
          ) : (
            <span>
              ❌ <strong>Error:</strong>{" "}
                ...
                <section>
                  ...
                  >
                    {variantsWithImage.map(({ node: variant }) => {
                      ...
                      // Conversione ProductImage GID → MediaImage GID per cleanup
